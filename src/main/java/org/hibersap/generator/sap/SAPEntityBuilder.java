...
public class SAPEntityBuilder {
	...
	public void createNew(final String className, final String javaPackage, final BapiMapping functionMapping) {
		...
		final JavaClassSource bapiClass = createJavaClass(className, javaPackage);


		// Adding BAPI Annotation
		final AnnotationSource<JavaClassSource> bapiAnno = bapiClass.addAnnotation(Bapi.class);
	 ...
	 */
	private JavaClassSource createJavaClass(final String className, final String javaPackage) {
...
//		javaClass.addField("/* \n * Generated by JBoss Forge Hibersap plugin\n */");//TODO Find a correct method to add a comment to the generated class
	 ...
	 */
	private void createConstructor(final JavaClassSource bapiClass, final Set<ParameterMapping> importParams) {
		...
		MethodSource<JavaClassSource> constructor = bapiClass.addMethod();
	 ...
	 */
	private void createParameters(final JavaClassSource bapiClass, final Set<? extends ParameterMapping> params,
		...
		for (final ParameterMapping param : params) {
			...
			if (paramType == ParamType.FIELD) {
				...
				final JavaClassSource structureClass = createStructureClass(javaPackage, param);
			...
			final AnnotationSource<JavaClassSource> paramterAnno = field.addAnnotation(Parameter.class);
	 ...
	 */
	private FieldSource<JavaClassSource> createSimpleField(final String name, final String type, final JavaClassSource javaClass) {
	 ...
	 * @return the created Java class
	 */
	private JavaClassSource createStructureClass(final String javaPackage, final ParameterMapping parameterMapping) {
		final String className = SAPEntityBuilder.convertFieldNameToClassName(parameterMapping.getJavaName());
		final JavaClassSource structureClass = createJavaClass(className, javaPackage);
		...
		for (final FieldMapping fieldMapping : fieldMappings) {
			final FieldSource<JavaClassSource> field = structureClass.addField();
			field.setName(fieldMapping.getJavaName());
			field.setType(fieldMapping.getAssociatedType());
			final AnnotationSource<JavaClassSource> annotation = field.addAnnotation(Parameter.class);
	...
	/**
	 * Workaround for Set cast
	 * @param parameterMapping
	 * @return
	 */
	private static Set<FieldMapping> cast(Set<ParameterMapping> parameterMapping){
		Set<FieldMapping> result = new HashSet<FieldMapping>();
		parameterMapping.forEach(item -> result.add((FieldMapping) item));
		return result;
	}
	/**
